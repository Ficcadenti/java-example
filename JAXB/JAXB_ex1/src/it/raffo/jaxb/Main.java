...
public class Main
{
	...
	public static void main(String[] args) throws TransformerConfigurationException
	{
		...
		File[] samplexsd = new File("D:\\temp\\").listFiles();

		// Create resulting folder
		if (!new File("SampleXML").exists())
		{
			new File("SampleXML").mkdir();
		}
		...
		for (int i = 0; i < samplexsd.length; i++)
		{
			...
			{ // Eliminate directories and make sure is a schema
				...
				XMLDocument sample = new XMLDocument(
						new StreamResult("SampleXML/"
								+ samplexsd[i].getName().substring(0, samplexsd[i].getName().indexOf(".")) + ".xml"),
						false, 4, null);
